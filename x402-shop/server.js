...
async function fetchPublic(raw, { maxBytes = MAX_BYTES, redirect = "follow" } = {}) {
  ...
  const res = await fetch(u.toString(), {
    redirect,
    headers: { "user-agent": "CashSprint-Fetch/1.5" },
...
function selector(sig) {
  const hash = new Bun.CryptoHasher("keccak256").update(sig).digest("hex");
...
function namehash(name) {
  ...
  for (const label of labels) {
    const labelHash = Buffer.from(new Bun.CryptoHasher("keccak256").update(label).digest());
    node = Buffer.from(new Bun.CryptoHasher("keccak256").update(Buffer.concat([node, labelHash])).digest());
  }
...
console.log(`x402-shop ${VERSION} listening on ${server.port}`);
