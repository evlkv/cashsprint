...
async function fetchPublic(raw, { maxBytes = MAX_BYTES, redirect = "follow" } = {}) {
  ...
  const res = await fetch(u.toString(), {
    redirect,
    headers: { "user-agent": "CashSprint-Fetch/1.46" },
...
const ROUTES = {
  ...
  "/pay/coap": {
    ...
    description: "Fetch RFC 8323 /.well-known/coap Alternative-Delivery for a public host. Distinct from CoRE Link Format /pay/core. $0.002 USDC on Base.",
    price: "0.002",
    params: [{ name: "host", required: true }],
    queryExample: { host: "example.com" },
    example: {
  ...
  "/pay/sbom": {
    ...
    description: "Fetch software bill of materials from /.well-known/sbom for a public host. Distinct from /pay/publiccode and /pay/csaf. $0.002 USDC on Base.",
    price: "0.002",
    params: [{ name: "host", required: true }],
    queryExample: { host: "example.com" },
    example: {
  ...
  "/pay/wot": {
    ...
    description: "Fetch W3C Web of Things /.well-known/wot Thing Description for a public host. Distinct from /pay/mcp and /pay/agent-card. $0.002 USDC on Base.",
    price: "0.002",
    params: [{ name: "host", required: true }],
    queryExample: { host: "example.com" },
    example: {
  ...
  "/pay/terraform": {
    summary: "Terraform registry discovery",
    description: "Fetch HashiCorp Terraform /.well-known/terraform.json service discovery for a public host. Distinct from /pay/nodeinfo and /pay/api-catalog. $0.002 USDC on Base.",
    price: "0.002",
    params: [{ name: "host", required: true }],
    queryExample: { host: "example.com" },
    example: {
  ...
  "/pay/license": {
    summary: "rel=license",
    description: "Extract HTML rel=license URLs from a public page. Distinct from software catalog /pay/publiccode. $0.001 USDC on Base.",
    price: "0.001",
    params: [{ name: "url", required: true }],
    queryExample: { url: "https://example.com" },
    example: { url: "https://example.com", license: "https://creativecommons.org/licenses/by/4.0/" },
    handler: async (q) => {
      const { url, text } = await fetchPublic(q.get("url"));
      const links = relHrefs(text, url, "license");
      return { url, license: links[0] || "", licenses: links.slice(0, 8) };
    },
  },
  "/pay/blockreceipts": {
    summary: "Base eth_getBlockReceipts",
    description: "eth_getBlockReceipts for the latest Base block (receipt count, first status, cumulative gas). Distinct from /pay/txid (one tx receipt), /pay/txlist, /pay/txindex, and /pay/txcount. $0.002 USDC on Base.",
    price: "0.002",
    params: [],
    queryExample: {},
    example: { network: "base", blockNumber: 1, receiptCount: 0, firstStatus: null, cumulativeGasUsed: "0" },
    handler: async () => {
      ...
      return {
        ...
        firstTxHash: first?.transactionHash || null,
        cumulativeGasUsed: first?.cumulativeGasUsed ? hexToBigInt(first.cumulativeGasUsed).toString() : "0",
        blockNumber: first?.blockNumber ? Number(hexToBigInt(first.blockNumber)) : null,
        blockHash: first?.blockHash || null,
      };
    },
  },
};
