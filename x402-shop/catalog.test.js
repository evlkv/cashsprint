...
test("shop version is 1.435.0", () => {
  expect(pkg.version).toBe("1.435.0");
  expect(source).toContain('const VERSION = "1.435.0"');
});
...
test("adds six new 1.435.0 pay routes", () => {
  for (const name of BATCH_1_435_0) {
    expect(routePaths).toContain(`/pay/${name}`);
  }
});
