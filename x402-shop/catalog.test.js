...
test("shop version is 1.181.0", () => {
  expect(pkg.version).toBe("1.181.0");
  expect(source).toContain('const VERSION = "1.181.0"');
});
...
test("adds six new 1.181.0 pay routes", () => {
  for (const name of BATCH_1_181_0) {
    expect(routePaths).toContain(`/pay/${name}`);
  }
});
