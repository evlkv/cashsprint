...
test("shop version is 1.7.0", () => {
  expect(pkg.version).toBe("1.7.0");
  expect(source).toContain('const VERSION = "1.7.0"');
});
...
test("keeps live and previous catalog paths", () => {
  for (const name of [...LIVE_1_4_0, ...BATCH_1_5_0, ...BATCH_1_6_0]) {
    expect(routePaths).toContain(`/pay/${name}`);
  }
});

test("adds six new 1.7.0 pay routes", () => {
  for (const name of BATCH_1_7_0) {
    expect(routePaths).toContain(`/pay/${name}`);
  }
});

test("new routes are not duplicates of earlier catalogs", () => {
  const prior = new Set([...LIVE_1_4_0, ...BATCH_1_5_0, ...BATCH_1_6_0]);
  for (const name of BATCH_1_7_0) {
