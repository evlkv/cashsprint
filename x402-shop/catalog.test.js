...
test("shop version is 1.312.0", () => {
  expect(pkg.version).toBe("1.312.0");
  expect(source).toContain('const VERSION = "1.312.0"');
});
...
test("adds six new 1.312.0 pay routes", () => {
  for (const name of BATCH_1_312_0) {
    expect(routePaths).toContain(`/pay/${name}`);
  }
});
