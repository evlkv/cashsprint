...
test("shop version is 1.307.0", () => {
  expect(pkg.version).toBe("1.307.0");
  expect(source).toContain('const VERSION = "1.307.0"');
});
...
test("adds six new 1.307.0 pay routes", () => {
  for (const name of BATCH_1_307_0) {
    expect(routePaths).toContain(`/pay/${name}`);
  }
});
