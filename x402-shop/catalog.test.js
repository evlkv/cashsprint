...
test("shop version is 1.490.0", () => {
  expect(pkg.version).toBe("1.490.0");
  expect(source).toContain('const VERSION = "1.490.0"');
});
...
test("adds six new 1.490.0 pay routes", () => {
  for (const name of BATCH_1_490_0) {
    expect(routePaths).toContain(`/pay/${name}`);
  }
});
