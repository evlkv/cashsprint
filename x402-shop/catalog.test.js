...
test("shop version is 1.352.0", () => {
  expect(pkg.version).toBe("1.352.0");
  expect(source).toContain('const VERSION = "1.352.0"');
});
...
test("adds six new 1.352.0 pay routes", () => {
  for (const name of BATCH_1_352_0) {
    expect(routePaths).toContain(`/pay/${name}`);
  }
});
