...
test("shop version is 1.205.0", () => {
  expect(pkg.version).toBe("1.205.0");
  expect(source).toContain('const VERSION = "1.205.0"');
});
...
test("adds six new 1.205.0 pay routes", () => {
  for (const name of BATCH_1_205_0) {
    expect(routePaths).toContain(`/pay/${name}`);
  }
});
