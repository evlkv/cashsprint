...
test("shop version is 1.178.0", () => {
  expect(pkg.version).toBe("1.178.0");
  expect(source).toContain('const VERSION = "1.178.0"');
});
...
test("adds six new 1.178.0 pay routes", () => {
  for (const name of BATCH_1_178_0) {
    expect(routePaths).toContain(`/pay/${name}`);
  }
});
