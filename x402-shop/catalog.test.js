...
test("shop version is 1.273.0", () => {
  expect(pkg.version).toBe("1.273.0");
  expect(source).toContain('const VERSION = "1.273.0"');
});
...
test("adds six new 1.273.0 pay routes", () => {
  for (const name of BATCH_1_273_0) {
    expect(routePaths).toContain(`/pay/${name}`);
  }
});
