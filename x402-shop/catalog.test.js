...
test("shop version is 1.457.0", () => {
  expect(pkg.version).toBe("1.457.0");
  expect(source).toContain('const VERSION = "1.457.0"');
});
...
test("adds six new 1.457.0 pay routes", () => {
  for (const name of BATCH_1_457_0) {
    expect(routePaths).toContain(`/pay/${name}`);
  }
});
