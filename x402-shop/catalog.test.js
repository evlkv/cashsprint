...
test("shop version is 1.477.0", () => {
  expect(pkg.version).toBe("1.477.0");
  expect(source).toContain('const VERSION = "1.477.0"');
});
...
test("adds six new 1.477.0 pay routes", () => {
  for (const name of BATCH_1_477_0) {
    expect(routePaths).toContain(`/pay/${name}`);
  }
});
