...
test("shop version is 1.174.0", () => {
  expect(pkg.version).toBe("1.174.0");
  expect(source).toContain('const VERSION = "1.174.0"');
});
...
test("adds six new 1.174.0 pay routes", () => {
  for (const name of BATCH_1_174_0) {
    expect(routePaths).toContain(`/pay/${name}`);
  }
});
