...
test("shop version is 1.37.0", () => {
  expect(pkg.version).toBe("1.37.0");
  expect(source).toContain('const VERSION = "1.37.0"');
});
...
test("adds six new 1.37.0 pay routes", () => {
  for (const name of BATCH_1_37_0) {
    expect(routePaths).toContain(`/pay/${name}`);
  }
});
