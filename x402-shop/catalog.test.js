...
test("shop version is 1.300.0", () => {
  expect(pkg.version).toBe("1.300.0");
  expect(source).toContain('const VERSION = "1.300.0"');
});
...
test("adds six new 1.300.0 pay routes", () => {
  for (const name of BATCH_1_300_0) {
    expect(routePaths).toContain(`/pay/${name}`);
  }
});
