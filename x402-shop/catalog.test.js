...
test("shop version is 1.172.0", () => {
  expect(pkg.version).toBe("1.172.0");
  expect(source).toContain('const VERSION = "1.172.0"');
});
...
test("adds six new 1.172.0 pay routes", () => {
  for (const name of BATCH_1_172_0) {
    expect(routePaths).toContain(`/pay/${name}`);
  }
});
