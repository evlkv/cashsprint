...
test("shop version is 1.62.0", () => {
  expect(pkg.version).toBe("1.62.0");
  expect(source).toContain('const VERSION = "1.62.0"');
});
...
test("adds six new 1.62.0 pay routes", () => {
  for (const name of BATCH_1_62_0) {
    expect(routePaths).toContain(`/pay/${name}`);
  }
});
