...
test("shop version is 1.168.0", () => {
  expect(pkg.version).toBe("1.168.0");
  expect(source).toContain('const VERSION = "1.168.0"');
});
...
test("adds six new 1.168.0 pay routes", () => {
  for (const name of BATCH_1_168_0) {
    expect(routePaths).toContain(`/pay/${name}`);
  }
});
