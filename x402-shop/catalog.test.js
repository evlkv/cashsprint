...
test("shop version is 1.274.0", () => {
  expect(pkg.version).toBe("1.274.0");
  expect(source).toContain('const VERSION = "1.274.0"');
});
...
test("adds six new 1.274.0 pay routes", () => {
  for (const name of BATCH_1_274_0) {
    expect(routePaths).toContain(`/pay/${name}`);
  }
});
