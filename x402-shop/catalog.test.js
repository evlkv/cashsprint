...
test("shop version is 1.378.0", () => {
  expect(pkg.version).toBe("1.378.0");
  expect(source).toContain('const VERSION = "1.378.0"');
});
...
test("adds six new 1.378.0 pay routes", () => {
  for (const name of BATCH_1_378_0) {
    expect(routePaths).toContain(`/pay/${name}`);
  }
});
