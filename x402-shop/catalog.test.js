...
test("shop version is 1.258.0", () => {
  expect(pkg.version).toBe("1.258.0");
  expect(source).toContain('const VERSION = "1.258.0"');
});
...
test("adds six new 1.258.0 pay routes", () => {
  for (const name of BATCH_1_258_0) {
    expect(routePaths).toContain(`/pay/${name}`);
  }
});
