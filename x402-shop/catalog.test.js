...
test("shop version is 1.340.0", () => {
  expect(pkg.version).toBe("1.340.0");
  expect(source).toContain('const VERSION = "1.340.0"');
});
...
test("adds six new 1.340.0 pay routes", () => {
  for (const name of BATCH_1_340_0) {
    expect(routePaths).toContain(`/pay/${name}`);
  }
});
