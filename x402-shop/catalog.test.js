...
test("shop version is 1.189.0", () => {
  expect(pkg.version).toBe("1.189.0");
  expect(source).toContain('const VERSION = "1.189.0"');
});
...
test("adds six new 1.189.0 pay routes", () => {
  for (const name of BATCH_1_189_0) {
    expect(routePaths).toContain(`/pay/${name}`);
  }
});
