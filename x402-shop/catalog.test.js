...
test("shop version is 1.367.0", () => {
  expect(pkg.version).toBe("1.367.0");
  expect(source).toContain('const VERSION = "1.367.0"');
});
...
test("adds six new 1.367.0 pay routes", () => {
  for (const name of BATCH_1_367_0) {
    expect(routePaths).toContain(`/pay/${name}`);
  }
});
