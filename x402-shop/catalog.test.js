...
test("shop version is 1.73.0", () => {
  expect(pkg.version).toBe("1.73.0");
  expect(source).toContain('const VERSION = "1.73.0"');
});
...
test("adds six new 1.73.0 pay routes", () => {
  for (const name of BATCH_1_73_0) {
    expect(routePaths).toContain(`/pay/${name}`);
  }
});
