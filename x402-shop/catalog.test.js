...
test("shop version is 1.299.0", () => {
  expect(pkg.version).toBe("1.299.0");
  expect(source).toContain('const VERSION = "1.299.0"');
});
...
test("adds six new 1.299.0 pay routes", () => {
  for (const name of BATCH_1_299_0) {
    expect(routePaths).toContain(`/pay/${name}`);
  }
});
