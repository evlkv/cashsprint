...
test("shop version is 1.420.0", () => {
  expect(pkg.version).toBe("1.420.0");
  expect(source).toContain('const VERSION = "1.420.0"');
});
...
test("adds six new 1.420.0 pay routes", () => {
  for (const name of BATCH_1_420_0) {
    expect(routePaths).toContain(`/pay/${name}`);
  }
});
