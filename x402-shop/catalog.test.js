...
test("shop version is 1.233.0", () => {
  expect(pkg.version).toBe("1.233.0");
  expect(source).toContain('const VERSION = "1.233.0"');
});
...
test("adds six new 1.233.0 pay routes", () => {
  for (const name of BATCH_1_233_0) {
    expect(routePaths).toContain(`/pay/${name}`);
  }
});
