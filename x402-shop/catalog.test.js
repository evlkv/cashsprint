...
test("shop version is 1.404.0", () => {
  expect(pkg.version).toBe("1.404.0");
  expect(source).toContain('const VERSION = "1.404.0"');
});
...
test("adds six new 1.404.0 pay routes", () => {
  for (const name of BATCH_1_404_0) {
    expect(routePaths).toContain(`/pay/${name}`);
  }
});
