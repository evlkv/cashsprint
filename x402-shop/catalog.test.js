...
test("shop version is 1.432.0", () => {
  expect(pkg.version).toBe("1.432.0");
  expect(source).toContain('const VERSION = "1.432.0"');
});
...
test("adds six new 1.432.0 pay routes", () => {
  for (const name of BATCH_1_432_0) {
    expect(routePaths).toContain(`/pay/${name}`);
  }
});
