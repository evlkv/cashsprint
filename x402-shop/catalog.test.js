...
test("shop version is 1.361.0", () => {
  expect(pkg.version).toBe("1.361.0");
  expect(source).toContain('const VERSION = "1.361.0"');
});
...
test("adds six new 1.361.0 pay routes", () => {
  for (const name of BATCH_1_361_0) {
    expect(routePaths).toContain(`/pay/${name}`);
  }
});
