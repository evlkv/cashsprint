...
test("shop version is 1.66.0", () => {
  expect(pkg.version).toBe("1.66.0");
  expect(source).toContain('const VERSION = "1.66.0"');
});
...
test("adds six new 1.66.0 pay routes", () => {
  for (const name of BATCH_1_66_0) {
    expect(routePaths).toContain(`/pay/${name}`);
  }
});
