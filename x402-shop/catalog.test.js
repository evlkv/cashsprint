...
test("shop version is 1.72.0", () => {
  expect(pkg.version).toBe("1.72.0");
  expect(source).toContain('const VERSION = "1.72.0"');
});
...
test("adds six new 1.72.0 pay routes", () => {
  for (const name of BATCH_1_72_0) {
    expect(routePaths).toContain(`/pay/${name}`);
  }
});
