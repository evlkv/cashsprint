...
test("shop version is 1.369.0", () => {
  expect(pkg.version).toBe("1.369.0");
  expect(source).toContain('const VERSION = "1.369.0"');
});
...
test("adds six new 1.369.0 pay routes", () => {
  for (const name of BATCH_1_369_0) {
    expect(routePaths).toContain(`/pay/${name}`);
  }
});
