...
test("shop version is 1.146.0", () => {
  expect(pkg.version).toBe("1.146.0");
  expect(source).toContain('const VERSION = "1.146.0"');
});
...
test("adds six new 1.146.0 pay routes", () => {
  for (const name of BATCH_1_146_0) {
    expect(routePaths).toContain(`/pay/${name}`);
  }
});
