...
test("shop version is 1.230.0", () => {
  expect(pkg.version).toBe("1.230.0");
  expect(source).toContain('const VERSION = "1.230.0"');
});
...
test("adds six new 1.230.0 pay routes", () => {
  for (const name of BATCH_1_230_0) {
    expect(routePaths).toContain(`/pay/${name}`);
  }
});
