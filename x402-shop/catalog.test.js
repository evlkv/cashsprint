...
test("shop version is 1.436.0", () => {
  expect(pkg.version).toBe("1.436.0");
  expect(source).toContain('const VERSION = "1.436.0"');
});
...
test("adds six new 1.436.0 pay routes", () => {
  for (const name of BATCH_1_436_0) {
    expect(routePaths).toContain(`/pay/${name}`);
  }
});
