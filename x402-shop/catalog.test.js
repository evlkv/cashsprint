...
test("shop version is 1.214.0", () => {
  expect(pkg.version).toBe("1.214.0");
  expect(source).toContain('const VERSION = "1.214.0"');
});
...
test("adds six new 1.214.0 pay routes", () => {
  for (const name of BATCH_1_214_0) {
    expect(routePaths).toContain(`/pay/${name}`);
  }
});
