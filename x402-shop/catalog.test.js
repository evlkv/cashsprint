...
test("shop version is 1.333.0", () => {
  expect(pkg.version).toBe("1.333.0");
  expect(source).toContain('const VERSION = "1.333.0"');
});
...
test("adds six new 1.333.0 pay routes", () => {
  for (const name of BATCH_1_333_0) {
    expect(routePaths).toContain(`/pay/${name}`);
  }
});
