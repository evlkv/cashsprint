...
test("shop version is 1.294.0", () => {
  expect(pkg.version).toBe("1.294.0");
  expect(source).toContain('const VERSION = "1.294.0"');
});
...
test("adds six new 1.294.0 pay routes", () => {
  for (const name of BATCH_1_294_0) {
    expect(routePaths).toContain(`/pay/${name}`);
  }
});
